...
{
    front = rear = nullptr;
    // set counter to 0 in constructor
    count = 0;
}
...
/*Post: The Queue is empty and all dynamic memory has been freed. */
{
    ...
    rear = nullptr;
    // set counter to 0 in destructor
    count = 0;
}
...
{
    front = rear = nullptr;
    // set counter to 0 in copy constructor
    count = 0;
...
/*Post: The Queue is set to be a copy of original. */
{
    ...
    rear = nullptr;
    // set counter to 0 in assignment operator
    count = 0;
...
/*Post: Returns true if the Queue is full, otherwise false. */
{
    ...
    if (test_node == nullptr) return true;
    delete test_node;
    return false;
}

int Extended_queue::size() const
/*Post: Returns the number of entries in the Extended_queue. */
{
    // return counter value instead of counting nodes
    return count;
}
...
/*Post: The Queue is empty and all dynamic memory has been freed. */
{
    ...
    rear = nullptr;
    // set counter to 0 in clear method
    count = 0;
}

Error_code Extended_queue::serve_and_retrieve(Queue_entry &item)
/*Post: The front item is copied to item, the front item is removed from the Queue,
        and a code of success is returned or a code of underflow is returned if the
        Queue is empty. */
{
    if (front == nullptr) return underflow;
    item = front->entry;
